...
from sqlalchemy import Table, Column, ForeignKey, String, Unicode, Integer, Index, Float, SmallInteger, Boolean#,text,PassiveDefault,ForeignKeyConstraint,DateTime,MetaData,Text #jorgeorpinel: unused
#from sqlalchemy.databases.postgres import PGArray #jorgeorpinel: unused
from sqlalchemy.orm import relation, sessionmaker, MapperExtension#,dynamic_loader,backref,column_property,deferred #jorgeorpinel: unused
...
#    from traceback import print_exc #debug
#    from sys import stdout          #debug
#    print_exc(file=stdout)          #debug

#    import sqlalchemy #jorgeorpinel: unnecesary due to following import
    import sqlalchemy.schema
    from sqlalchemy.orm import scoped_session#, mapper as sqla_mapper
...
#    mapper = Session.mapper #jorgeorpinel: this is deprecated... (solved below)
    ...
    #jorgeorpinel: this is the deprecation workaround: (http://www.sqlalchemy.org/trac/wiki/UsageRecipes/SessionAwareMapper)
...
#todo: but we currently do not cascade deletes from Artist to Track nor Artist to Album...
#IMPORTANT we do cascade from album to track but we *do* allow tracks to exist
# without an associated album or artist. Is that a good idea? I dunno...
...
mapper(Track, track, extension = DeleteFileMapperExtension(), properties = {
...
mapper(Album, album, properties = {
    'artists':relation(Artist, secondary = album_artist),
